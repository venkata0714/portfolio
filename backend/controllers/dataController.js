...
const getAllDocuments = async (collectionName, res) => {
  try {
    ...
    res.status(500).json({ message: `Error fetching documents from ${collectionName}` });
  }
};
...
const getDocumentByLink = async (collectionName, linkField, linkValue, res) => {
  try {
    ...
    console.error(`Error fetching document by link from ${collectionName}:`, error);
    res.status(500).json({ message: `Error fetching document by link from ${collectionName}` });
  }
};
...
const getInvolvementByLink = (req, res) => getDocumentByLink('involvementTable', 'involvementLink', req.params.involvementLink, res);

// Experiences
const getExperiences = (req, res) => getAllDocuments('experienceTable', res);
const getExperienceByLink = (req, res) => getDocumentByLink('experienceTable', 'experienceLink', req.params.experienceLink, res);

// Year In Reviews
const getYearInReviews = (req, res) => getAllDocuments('yearInReviewTable', res);
const getYearInReviewByLink = (req, res) => getDocumentByLink('yearInReviewTable', 'yearInReviewLink', req.params.yearInReviewLink, res);

// Honors Experiences
const getHonorsExperiences = (req, res) => getAllDocuments('honorsExperienceTable', res);
